...
class ReportController extends Controller
{
    ...
    public function balanceSheet(Request $request)
    {
        if(\Auth::user()->can('bill report'))
        {
            ...
            $types         = ChartOfAccountType::where('created_by',\Auth::user()->creatorId())->whereIn('id',[1,2,3])->get();
            ...
            foreach($types as $type)
            {
                ...
                foreach($subTypes as $subType)
                {
                    ...
                    $accountArray = [];
                    $totalAmount = 0; $debitTotal = 0; $creditTotal = 0; $accountSubType=''; $totalBalance=0;
                    foreach($accounts as $account)
                    {
                        ...
                        if($Balance != 0 || $journalItem->totalCredit != null ||$journalItem->totalDebit != null ||$journalItem->netAmount != null)
                        {
                            ...
                            $data['netAmount']    = $journalItem->netAmount + $Balance;
                            $accountArray[]       = $data;
...
//            -----------------------------
    ...
    public function trialBalanceExport(Request $request)
    {
        ...
        $data = Excel::download(new TrialBalancExport($totalAccounts , $start , $end, $companyName), $name . '.xlsx'); ob_end_clean();
            ...
            foreach($types as $type)
            {
                ...
                foreach($subTypes as $subType)
                {
                    ...
                    foreach($accounts as $account)
                    {
                        ...
                        // $creditBalance = Utility::getBalanceSheetCredit($getAccount->id,$start,$end);
                        // $totalCredit +=$creditBalance;

                        // $debitBalance = Utility::getBalanceSheetDebit($getAccount->id,$start,$end);
